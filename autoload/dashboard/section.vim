...
function! CompareById(i, j)
  return a:i["id"] > a:j["id"]
endfunction

function! s:set_section()
  let l = []
  for val in values(s:Section)
    call add(l, val)
  endfor

  let l:sorted = sort(l, "CompareById")

  for key in l:sorted
    let dashboard_{key["id"]} = g:dashboard#utils#set_custom_section(g:dashboard#utils#draw_center(key["description"]))
    call append('$',dashboard_{key["id"]})
    call dashboard#register(line('$'), key["id"], key["command"])
  endfor
endfunction